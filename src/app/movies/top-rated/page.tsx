...
export default function TopRatedMoviesPage() {
  ...
  const allGenres = Array.from(new Set(topRatedMovies.flatMap(movie => movie.genre)));

  // Filter and sort movies based on the selected options
  const filteredAndSortedMovies = [...topRatedMovies]
    .filter(movie => genreFilter === 'all' || movie.genre.includes(genreFilter))
    .sort((a, b) => {
      switch (sortBy) {
        case 'rating':
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
        <h1 className="text-2xl font-bold text-white mb-4 sm:mb-0">Top Rated Movies</h1>
        
        <div className="flex flex-wrap gap-3 items-center">
          {/* Genre filter */}
          <div className="relative">
            <label className="text-sm text-gray-400 mr-2">Genre:</label>
            <select
              value={genreFilter}
          ...
          <Link href={`/watch/${movie.slug}`} key={movie.id}>
            <div className="bg-[#181e23] rounded-lg overflow-hidden hover:shadow-lg hover:shadow-yellow-400/10 hover:scale-[1.03] transition-all group">
              <div className="relative h-64 sm:h-72">
                ...
                {index < 3 && (
                  <div className={`absolute top-2 left-2 w-7 h-7 flex items-center justify-center rounded-full text-sm font-bold ${
